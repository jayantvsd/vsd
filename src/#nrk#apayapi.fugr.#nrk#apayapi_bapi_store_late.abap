FUNCTION /nrk/apayapi_bapi_store_late.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*       l_user       TYPE xubname,
        ...
        l_apayno     LIKE /nrk/apayhd-apayno,
  ...
  IF datalate-barcode IS NOT INITIAL. " with barcode
    ...
    SELECT SINGLE object_key FROM bds_bar_in INTO l_object_id
      WHERE docutype EQ datalate-ar_object
        ...
        AND barcode  EQ datalate-barcode.
...
* Write document data to APay Center
...
* Update APay Center
  ...
  MOVE l_object_id(4) TO l_bukrs.
  MOVE l_object_id+4(10) TO l_belnr.
  MOVE l_object_id+14(4) TO l_gjahr.
...
* Link document to accounting record
...
*       BARCODE                     = ' '
    ...
    IF sy-subrc <> 0.
      RAISE document_linking_failed.
    ENDIF.

  ENDIF.

*Link document to APay Center record
