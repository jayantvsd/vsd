FUNCTION /nrk/apay_change_parked_doc_v2.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* Fill bdc item screen
    PERFORM bdc_write_items TABLES p_i_bdcdata USING lv_line wa_items header.
...
*** start change 01/13/2016
            ...
            IF NOT wa_vbsegs-matnr IS INITIAL.
              wa_items-matnr = wa_vbsegs-matnr.
            ENDIF.
            IF NOT wa_vbsegs-ausbk IS INITIAL.
              wa_items-bukrs = wa_vbsegs-ausbk.
            ENDIF.
            IF NOT wa_vbsegs-sgtxt IS INITIAL.
              wa_items-sgtxt = wa_vbsegs-sgtxt.
            ENDIF.
            IF NOT wa_vbsegs-kostl IS INITIAL.
              wa_items-kostl = wa_vbsegs-kostl.
            ENDIF.
            IF NOT wa_vbsegs-wrbtr IS INITIAL.
              wa_items-wrbtr = wa_vbsegs-wrbtr.
            ENDIF.
            IF NOT wa_vbsegs-zuonr IS INITIAL.
              wa_items-zuonr = wa_vbsegs-zuonr.
            ENDIF.
            IF NOT wa_vbsegs-zuonr IS INITIAL.
              wa_items-aufnr = wa_vbsegs-zuonr.
            ENDIF.
            IF NOT wa_vbsegs-shkzg IS INITIAL.
              wa_items-shkzg = wa_vbsegs-shkzg.
            ENDIF.

*** end change 01/13/2016

*** start change 01/08/2016

            wa_items-pspnr = wa_items-projk.

* convert wbs element and fill both line item fields
...
* Amount
  IF NOT pwa_items-wrbtr IS INITIAL.
    CLEAR plv_wrbtr.
* Convert amount to external format
    WRITE pwa_items-wrbtr CURRENCY pheader-waers TO plv_wrbtr NO-GROUPING.
    CONCATENATE 'ACGL_ITEM-WRBTR(' plv_line ')' INTO plv_field.
    PERFORM bdc_field  TABLES pp_i_bdcdata USING plv_field plv_wrbtr.
  ENDIF.
* Tax code
...
* Begin of change 12/13/2017
  IF NOT pwa_items-txjcd IS INITIAL.
    IF NOT pwa_items-txjcd EQ 'undefined'
      AND NOT pwa_items-txjcd EQ 'null'
      AND NOT pwa_items-txjcd IS INITIAL.
      CONCATENATE 'ACGL_ITEM-TXJCD(' plv_line ')' INTO plv_field.
      PERFORM bdc_field TABLES pp_i_bdcdata USING plv_field pwa_items-txjcd.
    ENDIF.
  ENDIF.
